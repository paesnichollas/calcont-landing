  ...
  return (
    <ScrollRevealSection id="galeria" className="mx-auto w-full max-w-6xl px-4 py-12 md:px-6 md:py-16" variants={stagger}>
      <div className="mb-6 space-y-2 md:mb-8">
        {galleryContent.title ? <h2 className="text-2xl font-semibold md:text-3xl">{galleryContent.title}</h2> : null}
          ...
          <CarouselContent className="-ml-2 md:-ml-4">
            {items.map((item, index) => (
              <CarouselItem key={item.id} className="basis-[84%] pl-2 sm:basis-[56%] md:pl-4 lg:basis-1/3">
                <Card className="overflow-hidden border-border/70 bg-card/70 transition-colors duration-200 hover:border-primary/30">
